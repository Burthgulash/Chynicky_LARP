...
export function init() {
  ...
  data.forEach((data, index) => {
    ...
    if (isOsmLinkHtmlRenderedMisto) {
      const htmlPopoverRenderedMisto = `
              <div class="popover" popover id="kalendar-misto-${index}">
                  <h2>Místo</h2>
                  <p>${data.misto.misto}</p>
                  <br>
                  <iframe class="mini-map" id="kalendar-mapa-${index}"
    ...
    htmlRenderedMisto = `
          ${
            data.misto.misto || typeof data.misto === "string"
              ...
              <${htmlTagRenderedMisto} class="datum-left misto-btn" ${isOsmLinkHtmlRenderedMisto ? `popovertarget="kalendar-misto-${index}"` : ""}>
  ...
  // Bind after rendering; the loader can run after DOMContentLoaded.
  document.querySelectorAll(".misto-btn[popovertarget]").forEach(button => {
    button.addEventListener("click", () => {
      const map = document.getElementById(button.getAttribute("popovertarget"))?.querySelector("iframe");
      if (map) map.src = map.src;
    });
  });
}
